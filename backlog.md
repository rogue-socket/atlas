...
- `[active 2026-05-09]` **Pattern A — 7 sparse single-test crashes still incomplete.** After fixing `QuadTreeNode` and `HighlightSyncBridge` (commits below), full suite is 54 pass / 0 fail / 7 incomplete (down from 19 yesterday). Remaining: `NovakExtractionTests.test_graph_novakData_roundTripsThroughCodable` + `test_graph_subtopicOfEdges_queryable`, `PDFSearchManagerTests.testClearHistory` + `testHistoryDedupesCaseInsensitiveAndTrims`, `ProjectsManagerTests.testPersistsAndLoadsProjects`, `RecentFilesManagerTests.testAddRecentFileDedupesAndKeepsMostRecentFirst`, `UndoRedoManagerTests.testUndoRedoStackTogglesAvailability`. Same `0x29eb33a70` malloc fingerprint. Surgical `nonisolated` on the implicated managers risks breaking SwiftUI binding (they're `@Published` `ObservableObject`s used as `@StateObject`). The structural fix — remove `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` from build settings, then add explicit `@MainActor` only to UI classes that need it — is the right long-term answer. Tests do pass when not scheduled first in their host (sparse, not class-uniform), so app correctness is verified.
