...
- [next] **Smoke-test the 2026-05-14 build-green-only changes live.** Bundle as one session. *Pause note 2026-05-14 18:1x:* build green, app launched from DerivedData, but the manual walk-through (annotation modes / extraction / search / RecentFiles) did not happen — paused to continue simplification work. App is still running; resume by stepping through the bundle below. (a) Backend drift reconciliation (`058936e`) — Claude temperature 0.1 → deterministic extraction; throw on UTF8/JSON decode failure. (b) PDFViewerView split (`a01e05c`) — exercise every annotation mode (highlight/underline/strikethrough/area highlight/ink/rectangle/circle/line/arrow/text/sticky note/select-and-drag), cursor changes per mode (fragile `updateNSView` path now in `PDFViewRepresentable.swift`), outline panel, annotation list panel. (c) GraphStore race fix (`42fe76c`) — encode now runs on caller thread; verify saves still complete during extraction without UI hiccups. (d) Map search debounce (`be82520`) — type into search; matching highlights should appear ~250ms after typing stops. (e) Annotation diff sync (`6f8131c`) — open an extracted document and verify highlights appear; trigger refresh and verify no flicker (no more wipe-then-readd). (f) RecentFilesManager (`d17ada2`) — open files, verify recent list updates without lag. Each has a one-commit revert if a regression shows up.
