...
final class HeadlessRunnerConfigTests: XCTestCase {
    ...
    func test_parse_acceptsDocOrderReverse() {
        ...
        XCTAssertEqual(c?.docOrder, "reverse")
    }

    func test_orderedFiles_reverseFlipsAlphaOrder() throws {
        let manager = ProjectsManager(
            storageURL: try makeTempDirectory().appendingPathComponent("projects.json"),
            bookmarker: URLDataBookmarker()
        )
        manager.createProject(name: "p", urls: [
        ...
        XCTAssertEqual(c.orderedFiles(from: project).map(\.displayName), ["B.pdf", "A.pdf"])
    }

    func test_orderedFiles_customListPreserved() throws {
        let manager = ProjectsManager(
            storageURL: try makeTempDirectory().appendingPathComponent("projects.json"),
            bookmarker: URLDataBookmarker()
        )
