...
- `AIServiceManager`: 6 backends — Claude, OpenAI, Gemini, Ollama, Claude-subscription (local Node sidecar, `claude-sidecar/server.mjs`, wrapping `claude -p` headless — no API key), and Codex Agent (local Python sidecar, `codex-agent-sidecar/server.py`, wrapping `codex exec --json`) — via the `AtlasModel` protocol. API keys in Keychain, response caching via SHA256 hash.
