...
| 3 | #22 `filteredNodeIDs` recomputed every keystroke | ✅ done | `be82520` | Two concerns separated: (a) cache — converted computed property to `@State`, recomputed only on query/graph change instead of every body eval (was running ~2x per render due to two call sites); (b) debounce — `.task(id: searchQuery)` produces `debouncedSearchQuery` 250ms after typing stops, with immediate-clear when the field is emptied. TextField binding unchanged so input responsiveness is preserved. |
| 3 | #23 `applyPersistentHighlights` rebuilds all | ✅ done | `6f8131c` | Storage moved from `[UUID: [PDFAnnotation]]` (keyed only by nodeID) to `[AnchorKey: PDFAnnotation]` over (documentURL, nodeID, pageIndex, decomposed bounds) — anchor-level identity is what lets the diff work. Apply now computes desired vs existing for this document, applies delta only, updates color in-place on kept keys. `refreshHighlights` collapses to a one-line alias. Side benefit: URL in the key fixes the latent wholesale-overwrite quirk (orphaned other-document annotations) that audit #21 also touches. All 5 `HighlightSyncBridgeTests` pass. |
| 3 | #25 `RecentFilesManager` re-resolves all on add/remove | ✅ done | `d17ada2` | `addRecentFile` dropped from ~2N security-scoped bookmark resolves + N file-existence checks to 0 resolves + 1 file check (for the newly-added URL). Dedup now matches against in-memory `recentFiles` instead of resolving each bookmark. `inaccessibleFiles: Set<Int>` indices stay coherent via shift bookkeeping on insert/remove/trim. `removeFiles(at:)` got the same in-memory treatment. All 5 `RecentFilesManagerTests` pass, including the async file-check tracer (preserved by the scoped per-URL check in `addRecentFile`). |
| 3 | #24 `addNode` `.info` log per node | partial — by design | `be9814b` | `merge(from:)` is silent (routed through `insert(_:)`); extraction/decode paths still log `.info` per node. Closing as partial-by-design — the per-node info log on extraction is a useful diagnostic when extraction quality is being investigated, and the user declined a global demote. Worth revisiting if log spam becomes a complaint. |
...
| 3 | #27 Obsidian/Markdown exporter overlap | won't-fix | — | Audit's own framing: "Marginal cost/benefit; flag for future." Two exporters share ~70% structure but differ at every customization point (link syntax, frontmatter, file layout). Extracting a shared base would mean either (a) parameterizing every difference, which dilutes both, or (b) a "common writer" that each exporter post-processes, which adds an abstraction without reducing the logic count. Closing — revisit only if a third exporter lands. |
| 3 | #28 `ScrollTracker` double observer | ✅ done | `2990b57` | Dropped `.PDFViewVisiblePagesChanged` (it duplicated `.PDFViewPageChanged` in `.singlePageContinuous` mode at a slower 200ms debounce). Plus added equality guards on the 3 `activeNodeID` write sites in `updateActiveNode` — `@Observable` setters notify downstream regardless of value change, so unconditional writes during same-page scrolls created redundant `DensityManager`/`MapCanvasRenderer` churn. |
| 3 | #29 Manual JSONSerialization in each backend | ✅ done | `01b3873` | Three private Decodable DTOs (`ClaudeResponse`, `OpenAIResponse`, `GeminiResponse`) replace the `as? [String: Any]` dict-walking. Parse collapses to one `JSONDecoder().decode(...)`. Diagnostic improvement: catch block now logs the underlying `DecodingError` (names the missing/wrong field) alongside the existing raw-response preview, so future API drift is easier to spot. |
| 3 | #30 File-leading WHAT comments + banners | guidance-only | — | Not a discrete commit — CLAUDE.md already says skip WHAT-comments. Trim opportunistically when editing a file for another reason. Closing this row; the guidance lives in CLAUDE.md, not in the survey backlog. |
| 3 | #16 MultiDocumentView 22 flat `@State` | ✅ done | `f492f40` | 9 of 22 props bundled into 3 nested state structs: `SidebarState` (section + queries), `CreateProjectSheetState`, `RenameProjectSheetState`. ~32 call sites converted via key-path access. SwiftUI's `$`-prefix bindings transparently project to nested members. |
| 3 | #17 Parameter sprawl on 5 APIs | won't-fix | — | The audit's "param count = sprawl" framing assumes pain from many callers repeating combinations. In practice each of the 5 flagged APIs has 1-2 callers and Swift's named args make params self-documenting. PDFViewRepresentable was the only borderline case (4 callbacks could bundle); declined because the call site uses inlined closures with logic — bundling adds indirection without reducing visible code. AlertManager would be strictly worse for many callers. Others are cosmetic with no caller pain. |
| 3 | #18 `RawConcept`/`RawEdge` stringly-typed | ✅ done | `810bac0` | Pushback on "decode directly with fallback decoder" — same `type: String` field decodes to two different ConceptType defaults (`.concept` for top-level, `.definition` for nested entity); context-aware fallback Codable can't express. New `String+EnumDecoding.swift` exposes `asConceptType(default:)` and `asEdgeType(default:)`; 6 call sites collapse. The `String` field stays — preserves the LLM's raw output verbatim. |
| 3 | #19 Three ad-hoc debouncers + Combine | ✅ done | `47977b7` | One of the three "manual debouncers" was actually dead code (`RecentFilesManager.fileCheckWorkItem`). The two real ones use the same cancel-then-reschedule pattern — no semantic mismatch as audit implied. Extracted `Debouncer` helper class (schedule + flush) and converted both. Combine sites stayed — Combine is the right tool when the source is a Publisher. |
| 3 | #21 Two `HighlightSyncBridge` instances | ✅ done | `3fe029c` | The "duplicate bridge" was a fresh `HighlightSyncBridge()` created in PDFViewerView's NavigateToPage handler just to call `findPassageRects`, a stateless function. Made `findPassageRects` and `whitespaceFlexibleMatch` static — codifies "no instance needed" and eliminates the hazard at the root. Tests followed same anti-pattern; 5 sites collapsed. All 5 HighlightSyncBridgeTests pass. |
